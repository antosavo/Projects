...
val x = pcaDF.select("pcaFeatures").collect.map(row=>row(0).asInstanceOf[DenseVector](0))

val y = pcaDF.select("pcaFeatures").collect.map(row=>row(0).asInstanceOf[DenseVector](1))

//val c = pcaDF.select("label").as[Double].collect.map(Map(0.0->"red",1.0->"blue"))

val c = pcaDF.select("label").as[Double].collect.map(Map(0.0 -> java.awt.Color.red, 1.0 -> java.awt.Color.blue,  2.0 -> java.awt.Color.green))
