#LASSO (L1) is a penalized model where we add the linear sum of
#the weights to our cost function.
#Depending on the regularization strength, certain weights will become zero.
#In contrast, ridge regression produces models in which most parameters are small but nonzero.
import pandas as pd
...
model = Lasso() #model = Lasso(alpha=1.0)

#We specified the hyperparameter space for the grid search.
parameters = {'alpha': (0.0001, 0.0025, 0.0050, 0.0075, 0.0100)}
