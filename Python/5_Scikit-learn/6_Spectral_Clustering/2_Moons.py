...
In Spectral Clustering each observation belongs to the cluster with which is better connected. The relationship between data points is rapressented by a graph, each edge is weighted by a similarity metric, the best partitions are the one where the weights that connect different clusters are as small as possible.
'''
import pandas as pd
import numpy as np
